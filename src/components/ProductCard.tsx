  ...
  return (
    <article className="rounded-2xl border border-gray-100 bg-white p-4 shadow-sm hover:shadow-md">
      ...
      <div className="mt-4 flex items-center justify-between">
        ...
        <Link href={`/shop/${product.slug}`} className="text-sm font-medium text-brand-700">View details</Link>
      </div>
      <div className="mt-4 border-t border-pink-100 pt-4">
        <Link
          href={`/shop/${product.slug}`}
          className="inline-flex min-h-11 w-full items-center justify-center rounded-full bg-brand-600 px-4 py-3 text-sm font-semibold text-white"
        >
          {product.inStock ? "Order product" : "View product"}
        </Link>
      </div>
    </article>
  );
