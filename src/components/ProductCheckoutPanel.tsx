...
export default function ProductCheckoutPanel({ product }: { product: Product }) {
  ...
  const submit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setMessage(
        isOnline
          ? "Checkout created. Continue with the payment link."
          : "Your pay-on-delivery order has been received. We will follow up before delivery."
      );
      setQty(1);
      setNotes("");
    } catch (error) {
  ...
  return (
    <aside className="rounded-2xl border border-pink-100 bg-white p-5 shadow-sm">
      ...
      <p className="mt-1 text-sm text-gray-600">
        Pay online with Paystack or choose pay on delivery. Orders are saved in Sedifex for store follow-up.
      </p>
      ...
      <form className="mt-5 grid gap-3" onSubmit={submit}>
        ...
        <select
          id="checkout-mode"
          value={mode}
          onChange={(event) => setMode(event.target.value as CheckoutMode)}
        ...
        <textarea
          ...
          placeholder="Preferred color, delivery instructions, etc."
        />
      ...
      {message ? (
        <p className={`mt-4 rounded-xl p-3 text-sm ${status === "error" ? "bg-red-50 text-red-700" : "bg-green-50 text-green-700"}`}>
