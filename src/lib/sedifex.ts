...
function resolveEndpoint(path: string, baseUrl: string, workspaceId?: string) {
  const endpoint = new URL(`${normalizeBaseUrl(baseUrl)}${path.startsWith("/") ? path : `/${path}`}`);
  const workspaceQueryParam = process.env.SEDIFEX_WORKSPACE_QUERY_PARAM;

  if (workspaceId && workspaceQueryParam) {
    endpoint.searchParams.set(workspaceQueryParam, workspaceId);
  }
...
async function fetchSedifexResource(path: string) {
  ...
  const workspaceId = process.env.SEDIFEX_WORKSPACE_ID;
  const requireWorkspaceId = process.env.SEDIFEX_REQUIRE_WORKSPACE_ID === "true";
  ...
  if (requireWorkspaceId && !workspaceId) {
    throw new Error("Sedifex workspace is required. Missing SEDIFEX_WORKSPACE_ID.");
  }

  const timeoutMs = toNumber(process.env.SEDIFEX_TIMEOUT_MS) ?? DEFAULT_TIMEOUT_MS;
  const endpoint = resolveEndpoint(path, baseUrl, workspaceId);
  const workspaceHeader = process.env.SEDIFEX_WORKSPACE_HEADER ?? "x-workspace-id";
